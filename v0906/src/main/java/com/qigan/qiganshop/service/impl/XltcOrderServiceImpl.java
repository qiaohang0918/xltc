...
import com.qigan.qiganshop.controller.utils.BalancePayController;
...
public class XltcOrderServiceImpl implements XltcOrderService {
    ...
    @Autowired
    TbOrderCommentService orderCommentService;
...
//        new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()) +  String.format("%03d", Long.parseLong(new SimpleDateFormat("S").format(new Date()))) + RandomStringUtils.random(3, false, true)
        ...
        if (!StringNotNull.check(userChooseTime)) {
            ...
            } else if(userChooseTime.startsWith("YS")){
            	order.setUserChooseTime(userChooseTime);
            }else {
                resultMap.put("code", ResultCode.FAIL.res_code());
...
//                return jr.jsonResultData(ResultCode.FAIL.res_code(), "配送时间异常,仅可以传输以下内容[1小时内,2小时内,今天:17:00-18:00,明天:17:00-18:00]!");
        ...
        cart.setMobileId(groupOrder.getMobileId());
...
//####################################################################################################################################################
        ...
        //查询较为宽泛的限购商品
        Set<String> limitedCodesCasue = findLimitedCodesCasue();
        //查询预售商品
        Set<String> preSellGoods = findPreSellGoods();
        for (Goods goods : canBuy) {
            ...
            if (one != null) {
                ...
                //判断是否是预售商品
                if(preSellGoods.contains(one.getCode())){
                    //是预售商品 校验可行
                    if(canBuy.size()>1){
                        //还有额外商品，不可行
                        resultMap.put("code", ResultCode.FAIL.res_code());
                        resultMap.put("message", "[" +one.getName()+ "]是预售商品，只能单独购买!");
                        return resultMap;
                    }else {
                        //无额外商品 预售可行 ,打标预售，发单时间
                        String duringAfter = this.findPreSellTime(one.getCode());
                        if(duringAfter == null || duringAfter.split("@").length<2){
                            resultMap.put("code", ResultCode.FAIL.res_code());
                            resultMap.put("message", "[" +one.getName()+ "]商品是预售商品，缺少配置预售截止时间!");
                            return resultMap;
                        }
                        String sellTime = duringAfter.split("@")[1].replace("_"," ");
                        try {
                            LocalDateTime parse = LocalDateTime.parse(sellTime, TimeFormat.normalDateTimeFormater);
                            //校验以过期
                            if(parse.isBefore(LocalDateTime.now())){
                                resultMap.put("code", ResultCode.FAIL.res_code());
                                resultMap.put("message", "预售商品[" +one.getName()+ "]已过期，请重新选择!");
                                return resultMap;
                            }
                        }catch (Exception e){
                            resultMap.put("code", ResultCode.FAIL.res_code());
                        ...
                        //预售  预售时间  期号
                        resultMap.put("isPreSellGoods" , "1");
                        resultMap.put("preSellTime" , sellTime);
                        resultMap.put("level" , duringAfter.split("@")[0]);
                    }
                }
                ...
                //校验比较宽泛的限购商品的数量
                if(limitedCodesCasue.contains(one.getCode())){
                    if(orderItem.getCount()>1) {
                        resultMap.put("code", ResultCode.FAIL.res_code());
                        resultMap.put("message", "[" + one.getName() + "]商品每人每次只能购买一件，但可多次购买，请重新选择!");
                        return resultMap;
                    }
                }
                // 计量单位
        ...
        PageResult pageCoupon = resultCart.getCouponList(); 
        
        
        if (coupons != null && coupons.size() > 0) {
        	if(pageCoupon != null && pageCoupon.getTotal() == 0){
            	throw XltcRuntimeException.throwable("该优惠券无法使用,请检查订单商品是否符合规则");
            }
            for (Coupon coupon : coupons) {
            	List<UserCoupon> usercps = couponMapper.findUserCoupons(user.getUserId(), coupon.getCouponId()); 
            	if(usercps != null && usercps.size() > 0){
            		UserCoupon usercp = usercps.get(0);
            		if(usercp != null){
            			UserCoupon userCoupon = new UserCoupon();
            			userCoupon.setIsUse("1");
            			userCoupon.setUserCouponId(usercp.getUserCouponId());
    ...
    /**
     * 查询规则宽泛的限购商品、
     * 规则：每次购买只能买一个，但可以购买多次！
     * @return
     */
    public Set<String> findLimitedCodesCasue(){
    ...
    /**
     * 查询预售时间  duringBefore_duringAfter
     * @param code
     * @return
     */
    public String findPreSellTime(String code){
        String duringAfter = jedisUtil.getFromHash(RedisConstant.PRESELL_GOODS_CODE, code);
        if(!StringNotNull.check(duringAfter))
            return null;
        else
             return duringAfter;
    }
    ...
    public JsonResult  tailSubmit(Map<String,Object> resultMap){
        ...
        //校验预售
        Object isPreSellGoods = resultMap.get("isPreSellGoods");
        if(isPreSellGoods!=null){
            String preSellTime = (String) resultMap.get("preSellTime");
            String level = (String) resultMap.get("level");
            //订单打标
            order.setTag("preSell");
            order.setLevel(level);
            //添加预售订单记录
            TbPresellOrder presellOrder = new TbPresellOrder();
            presellOrder.setPresellorderid(order.getOrderId());
            presellOrder.setCreatetime(dateFormat.format(new Date()));
            presellOrder.setIspushed("0");
            presellOrder.setPreselltime(preSellTime);
            //预售批次  code  buyCount
            presellOrder.setLevel(level);
            Item one = itemService.findOne(byItems.get(0).getItemId());
            presellOrder.setCode(one.getCode());
            presellOrder.setButCount(String.valueOf(byItems.get(0).getCount()));
            int i = preSellOrderService.insert(presellOrder);
        }
        order.setStatus("0");
...
//        }
       ...
       /* if(userCoupons == null)
        	throw XltcRuntimeException.throwable("优惠券使用异常，请稍后再试");*/
    ...
    public Map<String, Object> payInfo(String out_trade_no, String payType, String orderId) {
        ...
        if (pay == null) {
            if ("zhifubao".equals(payType)) {
                ...
                //账户支付，不做处理（走到这里已经成功插入pay，且no exception!）
...
//            resultMap.put("pay", pay);
    ...
    @Override
    public JsonResult optPreSellOrder(String orderId, Order one){
        if (!"1".equals(one.getStatus())) {
            one.setStatus("1");
            orderMapper.update(one);
        }
...
//        conditionalMap.put("minMoney",minMoneyDouble);
     ...
     */
    @Override
    public List<Order> findByOrderForExport(Order order, String beginTime, String endTime, Integer page, Integer rows, String rule) {
    ...
    @Override
    public List<ClueUserInfo> selectCLueUserInfo() {
        return userMapper.selectCLueUserInfo();
    }
    ...
    public String pullToGuanYi(String OrderId) {
        ...
        if("preSell".equals(order.getTag())){
            //查询预售订单发货时间
            TbPresellOrder presellOrder =  preSellOrderService.findByOrderId(order.getOrderId());
            if(presellOrder != null){
                //设置 管易订单的二次备注 和 附加信息
                List<OrderItem> byOrderId = orderItemService.findByOrderId(order.getOrderId());
                Item one = itemService.findOne(byOrderId.get(0).getItemId());
                String code = one.getCode();
                //查询预计发货日期
                TbPresellGoods presellGoods = preSellGoodsService.findPreSellGoodsByCodeAndLevel(code,order.getLevel());
                String preSendTime = presellGoods == null ? " " : presellGoods.getPresendtime();
                guanYiOrder.setSeller_memo_late("预售单,批次："+order.getLevel()+",预计[ "+preSendTime+" ]发");
                guanYiOrder.setExtend_memo(order.getBuyerMemo());
            }
        }
        // 添加用户信息
    ...
    public JsonResult findByStatus(String userId, String status, Integer page, Integer rows) {
        ...
        for (Order one : orderList) {
            ...
            if(one != null){
            	map.put("orderStatus", OrderStatus.getOrderStatus(one.getStatus()));
            	map.put("buyerMemo", one.getBuyerMemo());
            }
            result.add(map);
        }
     ...
     */
    @Override
    public Integer updateComment(String userPhone, String orderId, Integer comment, Integer orderLevel, String context) {
        ...
        TbOrderComment tb = new TbOrderComment();
        tb.setOrderid(orderId);
        tb.setDeliveryLevel(new BigDecimal(comment));
        tb.setGoodsLevel(new BigDecimal(orderLevel));
        tb.setUserPhone(userPhone);
        tb.setCommentContext(context);
        orderCommentService.save(tb);
        return updateComment;
    }

	@Override
	public PageResult findOrderList(Order order) {
		// TODO Auto-generated method stub
		order.startPageHelper();
		PageInfo<?> result = new PageInfo<>(orderMapper.findOrderList(order));
