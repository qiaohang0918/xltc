...
public interface GoodsMapper {
    ...
    @Select("select goodsId from tb_goods where code = #{code} limit 0 , 1 ")
    String findGoodsIdByCode(@Param("code") String code);


    @Select("select DISTINCT goodsId from tb_goods where code in ( ${codesSqlFormat} )  ")
    List<String> findGoodsIdsByCodes( @Param("codesSqlFormat") String codesSqlFormat);

    @Select("select name from tb_goods where code = #{code} limit 0 , 1 ")
   ...
   Set<String> findCategoryCode(@Param("ids") List<String> codes);
    ...
    @Select({
    	"select categoryCode from tb_goods where goodsId = #{goodsId}"
    })
    List<String> findGoodsCateById(String goodsId);

    @Select({"select * from tb_goods where 1=1  ${conditions}" ,
            ""})
    List<Goods> findGoodsByConditions(@Param("conditions") String conditions);

    @Select({"select a.orderId from tb_order a " ,
            "left join tb_order_item b on a.orderId = b.orderId " ,
            "where b.goodsId = #{goodsId} " ,
            "and a.userId = #{userId} " ,
            "limit 0 , 1 "})
    String checkUserLimitedRecord(@Param("userId") String userId, @Param("goodsId") String goodsId);
}