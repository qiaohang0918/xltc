...
public class WxpayController {
	...
	@Value("${deposit.member.free.given}")
	private String freeMemberDays;
...
//						}
				...
				jedisUtil.removeElementFromHash(RedisConstant.NEW_USER_FIRST_ORDER, order.getUserId());
			}
		} catch (Exception e) {
...
//			String notify_url = SERVER_URL + "qiganshop/app/wxpay/depositNotify.do";
			...
			//插入支付订单记录（不可退款）
			//......
			String total = ServletRequestUtils.getStringParameter(request, "total_fee","0.00");
			BigDecimal total1 = new BigDecimal(total);
			BigDecimal total2 = new BigDecimal(100);
			int total_fee = total1.multiply(total2).intValue();
					...
					if( i>0 ){
						...
						try {
							if("balance".equals(tbDeposit.getDepositContent())){
								...
								//下发账户充值会员（充值就送3天会员）
								boolean memberDepositResult = this.givenMemberDays(userId, "3");
								//充值送优惠卷
								boolean givenCoupons = this.givenDepositCoupons(userId);
								//记录info  异常日志  微信充值支付成功，但下发账户余额失败！
								Info info = new Info();
								info.setOrderId(tbDeposit.getOrderId());
								info.setUserId(userId);
								info.setInfo("微信充值(balance)成功！本次支付金额："+depositYuan);
								infoService.add(info);
							}
							if("member".equals(tbDeposit.getDepositContent())){
								//执行会员业务.......（deys > config > default）
								boolean memberDepositResult = this.givenMemberDays(userId, "30");
								//记录info  异常日志  微信充值支付成功，但下发账户余额失败！
								Info info = new Info();
								info.setOrderId(tbDeposit.getOrderId());
								info.setUserId(userId);
								//可根据 memberDepositResult 插入不同的会员充值信息(暂略...)
								info.setInfo("微信充值会员(member)成功！本次支付金额："+depositYuan);
								infoService.add(info);
							}
							//删除redis
							...
							info.setOrderId(tbDeposit.getOrderId());
							info.setUserId(userId);
							info.setInfo("微信充值支付成功，但业务（"+tbDeposit.getDepositContent()+"）执行失败,实付金额:"+depositYuan);
							infoService.add(info);
						}
					}
					//响应微信成功请求
	 ...
	 * @return
	 */
	public boolean givenDepositCoupons(String userId) {
		Set<String> couponIds = this.fidndDepositCoupons();
		if(couponIds != null){
	 ...
	 * @return
	 */
	public Set<String> fidndDepositCoupons() {
	...
	/**
	 * 充值会员天数(when days = null then config then default)
	 * @param userId
	 * @param days
	 * @return
	 */
	public boolean givenMemberDays(String userId,String days) throws ParseException {
		//查询当亲用户
		AppUser user = userService.getAppUserById(userId);
		//计算会员天数
		String realGivenDays = null;
		this.opratorMemberDays(realGivenDays,days);
		if(user!=null){
			//会员信息新建 / 延期
			if("1".equals(user.getMember())){
				//校验是否已经过期(定时任务未来得及修改member状态)
				if(this.isMemberExpired(user)){
					//会员已过期
					//新建会员信息
					boolean result = this.newMemberInfo(user, realGivenDays);
				}else {
					//会员延期
					String mendTime = user.getMendTime();
					LocalDateTime parse = LocalDateTime.parse(mendTime, TimeFormat.normalDateTimeFormater);
					String formatEndTime = parse.plusDays(Long.parseLong(realGivenDays)).format(TimeFormat.normalDateTimeFormater);
					user.setMendTime(formatEndTime);
					int i = userService.updateMemberInfo(user);
				}
			}else if(StringNotNull.check(user.getMbeginTime(),user.getMendTime())){
				//当前会员业务暂停  (不是首次享受会员服务) , 更新会员状态
				//新建会员信息
				boolean result = this.newMemberInfo(user, realGivenDays);
			}else {
				//新入会员
	 ...
	 * @return
	 */
	public boolean newMemberInfo(AppUser user,String realGivenDays){
		user.setMember("1");
		user.setMbeginTime(TimeFormat.nomalFormat.format(new Date()));
		LocalDateTime emdTime = LocalDateTime.now().plusDays(Long.parseLong(realGivenDays));
		String formatEndTime = emdTime.format(TimeFormat.normalDateTimeFormater);
		user.setMendTime(formatEndTime);
		int i = userService.updateMemberInfo(user);
	 ...
	 * @param days
	 */
	public void opratorMemberDays(String realGivenDays,String days){
		//校验days  为空则走配置时间，配置为null， 则走默认时间
		if(!StringNotNull.check(days)) {
			if (!StringNotNull.check(freeMemberDays)) {
				//默认给三天会员
				realGivenDays = "3";
			} else {
				if (this.checkIntString(freeMemberDays)) {
					realGivenDays = freeMemberDays;
				} else {
					realGivenDays = "3";
				}
			}
		}else {
			if(this.checkIntString(days)){
				realGivenDays = days;
			}else {
				if (!StringNotNull.check(freeMemberDays)) {
					//默认给三天会员
	 ...
	 * @return
	 */
	public boolean checkIntString(String waitingCheck){
		if(!StringNotNull.check(waitingCheck))
			return false;
		try {
			Integer.parseInt(waitingCheck);
			return true;
		}catch (Exception e){
			return false;
		}
	 ...
	 */
	public boolean isMemberExpired(AppUser user) throws ParseException {
		if(user == null)
			return true;
		if(!StringNotNull.check(user.getMember()) || "0".equals(user.getMember()))
			return true;
		String mendTime = user.getMendTime();
		if(!StringNotNull.check(mendTime)){
			return true;
		}
		Date parse = TimeFormat.nomalFormat.parse(mendTime);
