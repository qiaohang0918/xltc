...
public class BalancePayController {
    ...
    public JsonResult pay(String token, String total, String orderId, String paypassword, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            if (user != null) {
                if("1".equals(user.getIsFreeze())){
                    return jr.jsonResultData(ResultCode.FAIL.res_code(), "该用户余额支付被冻结！");
                }
                String ppwdb = user.getPaypassword();
                String ppw = DigestUtils.md5Hex(paypassword);
                System.err.println(ppwdb);
                System.err.println(ppw);
                System.err.println(paypassword);
                if (!ppw.equals(ppwdb)) {
...
//                                }
                        ...
                        jedisUtil.removeElementFromHash(RedisConstant.NEW_USER_FIRST_ORDER, o.getUserId());
                    }
