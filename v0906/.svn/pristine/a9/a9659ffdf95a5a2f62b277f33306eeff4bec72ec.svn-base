...
public class AppGoodsController {
    ...
    @ApiOperation(
            ...
            notes = "查询单个商品",
            httpMethod = "POST")
    public JsonResult findOne(String token, String goodsId,String coordinate) {
        Goods one = service.findOne(token, goodsId);
        if (one == null) {
            return jr.jsonResultData(ResultCode.SUCCESS.res_code(), "该商品已下架");
        }
        //封装预售商品库存 goods.stockNum
        if (one != null) {
            if(one.getItems() == null || one.getItems().size()<=0){
                return jr.jsonResultData(ResultCode.FAIL.res_code(), "商品已下架!");
            }
            if(StringNotNull.check(coordinate)) {
                ...
                one.setWarehouseId(wareId);
                if(one.getPreSellInfo() != null) {
                    List<Item> items = one.getItems();
                    if(!SqlConstructUtils.nullList(items)) {
                        for (Item item : items) {
                            if (wareId.equals(item.getWarehouseId())) {
                                if (item.getStockNum() == null || item.getStockNum().intValue() <= 0) {
                                    one.setStockNum(0);
                                } else {
                                    one.setStockNum(item.getStockNum());
                                }
                            }
                        }
                    }
