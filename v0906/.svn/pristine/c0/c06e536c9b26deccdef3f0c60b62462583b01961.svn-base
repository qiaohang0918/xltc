...
import com.qigan.qiganshop.pojo.synchronization.ResultGoods.ItemsBean;
...
public class XltcGoodsServiceImpl implements XltcGoodsService {
...
//    		writeLocalStrOne(fileStr, "D:\\aaa.txt");
		...
		int page = 1, size = 100;
		ResultGoods results = gy.get(itemsMethod, null, 1, null, ResultGoods.class, null);
	...
	@Override
	public void syncGoodsByCode(String... codes) {
		// TODO Auto-generated method stub
		if(StringUtils.isAllBlank(codes))
			throw XltcRuntimeException.throwable("请至少输入一个code");
		List<Item> litems = new ArrayList<>();
		List<Goods> listGoods = new ArrayList<>();
		...
		for (String code : codes) {
			ResultGoods results = gy.get(itemsMethod, code, ResultGoods.class, null);
			List<ItemsBean> list = results.getItems();
			if(list == null)
				throw XltcRuntimeException.throwable("商品【" + code + "】管易不存在");
			if(list.size() > 1)
				throw XltcRuntimeException.throwable("商品【" + code + "】管易出现重复");
			ItemsBean bean = list.get(0);
			if (bean.isCombine() || bean.isDel())
				continue;
			Goods goods = new Goods();
			goods.setGoodsId(bean.getId());
