...
public class LabelServiceImpl implements LabelService {
	...
	public Integer delete(String[] ids) {
		...
		for (String labelId : ids) {
			count += mapper.delete(labelId);
			//删除label下的所有商品
			int result = mapper.deleteLableGoodsByLableId(labelId);
		}
		return count;
	}
	...
	@Override
	public XltcResult findUnitGoodsByCateId(String cateId, CommonPage page) {
		// TODO Auto-generated method stub
		page.startPageHelper();
		List<XltcGoodsModel> list = unionMapper.findUnitGoodsByCateId(cateId);
		PageInfo<XltcGoodsModel> result = new PageInfo<>(list);
		return XltcResult.ok(new PageResult(result.getTotal(), list));
	}

	@Override
	public PageResult findPageAllByLabelId(String lId, String coni, CommonPage page) {
		// TODO Auto-generated method stub
		List<?> list = this.findPageByLabel(lId, coni, page);
