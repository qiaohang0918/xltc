...
import com.qigan.qiganshop.util.filter.LogIntercept;
...
//		if (USER_FREE.equals(cou.getType()) || NEW_USER.equals(cou.getType())) {
			...
			if (StringUtils.isBlank(end) || end.indexOf(".") != -1 || "0".equals(end))
				end = "1";
...
//			endDay = Long.parseLong(end) * 24 * 3600;
			...
			String disDateStr = nowDate.plusDays(Long.parseLong(end) - 1).toString() + " 23:59:59";
			LocalDateTime endTime = LocalDateTime.parse(disDateStr, DF);
			userCoupon.setStartTime(DF.format(LocalDateTime.now()));
			userCoupon.setDisableTime(disDateStr);
			endDay = Duration.between(LocalDateTime.now(), endTime).toMillis() / 1000;
		} else {
			try {
		...
		jedis.removeElementFromHash(RedisConstant.USER_ALERT_FLAG, userId);//领取优惠券后移除弹窗标记，方便出现广告后弹窗
		return add;
	}
	
	public static void main(String[] args) {
		LocalDate nowDate = LocalDate.now();
		String disDateStr = nowDate.plusDays(-1).toString() + " 23:59:59";
		LocalDateTime endTime = LocalDateTime.parse(disDateStr, DF);
//		userCoupon.setStartTime(nowDate + "00:00:00");
//		userCoupon.setDisableTime(disDateStr);
		Long endDay = Duration.between(LocalDateTime.now(), endTime).toMillis() / 1000;		
		System.err.println(endDay);
		System.err.println(DF.format(LocalDateTime.now()));
		System.err.println(disDateStr);
	}
...
//		System.err.println(JsonUtils.writeValueAsString(list));
		for (XltcUserCouponModel model : list) {
			if(model != null && (NEW_USER.equals(model.getType()) 
					|| CouponType.ForSend.getValue().equals(model.getType())
					|| CouponType.NewUserFirstOrder.getValue().equals(model.getType())))
				continue;
...
//		String mobile = jedis.getFromHash(RedisConstant.NEW_MOBILE, mobileId);//为空时为新设备
	...
	@Override
	public List<?> findAllCoupon4App(String token, String goodsId, String mobileId, String index) {
		// TODO Auto-generated method stub
		Goods goods = goodsMapper.findOne(goodsId);
		if(goods == null)
			return new ArrayList<>();
		String userId = checkToken(token);
		boolean flagCoupon = StringUtils.isNotBlank(index) ? true : false;
		List<String> list = null;
		if(flagCoupon){
			list =  CouponType.showCouponList(true);
		}else{
			list = CouponType.showCouponList4App(true);
		}
		String value = jedis.getFromHash(RedisConstant.NEW_USER_FIRST_ORDER, userId);//不为空则为新人首单
		String mobile = StringUtils.isNotBlank(mobileId) ? jedis.getFromHash(RedisConstant.NEW_MOBILE, mobileId) : IDUtils.genItemId();//为空时为新设备
		//新人首单券同时满足  1 用户首单可领取， 2 新设备可领取
...
//		}
	...
	@Override
	public void sendCoupon2User(String no, String[] userIds, String... couponIds) {
		// TODO Auto-generated method stub
		if(StringUtils.isAllBlank(userIds) || StringUtils.isAllBlank(couponIds))
			throw XltcRuntimeException.throwable("参数不完整，请检查参数");
		for (String userId : userIds) {
			for (String couponId : couponIds) {
				UserCoupon userCoupon = new UserCoupon(userId, couponId, "1", no);
				this.add(userCoupon, false);
			}
		}
	}

	@Override
	public AppUser chenUserCanSend(String userPhone) {
		// TODO Auto-generated method stub
		if(StringUtils.isBlank(userPhone))
			throw XltcRuntimeException.throwable("手机号为空");
		AppUser user = userService.findAppUserByPhone(userPhone);
		if(user == null)
	...
	@Override
	public PageResult findSendCouponAll(UserCoupon coupon) {
		// TODO Auto-generated method stub
		coupon.startPageHelper();
		PageInfo<XltcCouponModel> list = new PageInfo<>(mapper.findSendReport(coupon));
		return new PageResult(list.getTotal(), list.getList());
	}
