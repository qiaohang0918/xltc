...
public class XltcExpladServiceImpl implements XltcExpladService {
    ...
    @Override
    public List<TbExpland> exportExcelFindExplandUsers(String startTime, String endTime, String phone) {
        StringBuffer timebuffer = new StringBuffer(" ");
        StringBuffer phonebuffer = new StringBuffer(" ");
        if(StringNotNull.check(startTime)){
            timebuffer.append(" and registTime >= '"+startTime+"' ");
        }
        if(StringNotNull.check(endTime)){
            timebuffer.append(" and registTime <= '"+endTime+"' ");
        }
        if(StringNotNull.check(phone)){
            phonebuffer.append(" and a.phone = '"+phone+"' ");
        }
        List<TbExpland> list = null;
        try {
            list = explandMapper.exportExcelFindExplandUsers(phonebuffer.toString(),timebuffer.toString());
        }catch (Exception e){
        ...
        if(!SqlConstructUtils.nullList(list)){
            for (TbExpland expland : list) {
                ...
                //查询其下扩展用户下单的用户数量
                int alreadyOrderedCount = explandMapper.findHisExplandUserWhichHavingOrdered(explanduserid,timebuffer.toString());
